...
// $dlugosc_tablicy = htmlspecialchars($_POST['dlugosc_tablicy']);

// $rekord_do_usuniecia = 0;

/*
 * for ($i=1 i<=$dlugosc_tablicy; $i++) {
 *
 * if ($tabela[$i] != '') {
 * $rekord_do_usuniecia = $tabela[$i];
 * }
 * }
 */

// $rekord_do_usniecia = $tabela[2];

// $rekord_do_usniecia = htmlspecialchars($_POST['rekord_do_usniecia']);

sprawa_dodaj_lub_aktualizuj_strona_11_a_1 ( $sprawa_id, $pod_wplywem, $uzywki_id, $w_pojezdzie, $rola, $typ_pojazdu, $typ_pojazdu_inny, '', $pozycja, $pozycja_inna, $pasy, $wspolwlasnosc, $wiedza_o_stanie_kierowcy, $wiedza_o_upr_kierowcy );
...
sprawa_dodaj_lub_aktualizuj_strona_11_a_2 ( $sprawa_id, $stan_leczenia, $lecz_data_zakonczenia, $lecz_data_planowany, $od_kiedy_l4, $do_kiedy_l4 );
...
$przebieg_leczenia = sprawa_dodaj_lub_aktualizuj_strona_11_a_3 ( $sprawa_id, $szpital, $przychodnia, $przychodnia_data );

$miejsce_zabiegow = htmlspecialchars ( $_POST ['miejsce_zabiegow'] );
$zabiegi_data = htmlspecialchars ( $_POST ['zabiegi_data'] );

sprawa_dodaj_lub_aktualizuj_placowki ( $przebieg_leczenia ['sprawa_przebieg_leczenia_id'], $miejsce_zabiegow, $zabiegi_data );
