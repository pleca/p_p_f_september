...
$stopka = 'PG-2-19-F1/2018-05-24';
//$stopka = 'PG-2-19-F1/2018-02-12';
$nr_strony = 1;
($liczba_wlascicieli == 1 || $liczba_wlascicieli == 2) ? $strony = 3 : $strony = $liczba_wlascicieli+1;
...
<div class="pdf_strona">
    <div class="pdf_strona_pierwsza_naglowek_bls">
        ...
        <div class="pdfs_tytul_dokumentu pdf_tytul_bls">
            ...
            <p class="margin_b_0 font_w_700 font_size_22 text_a_center">POWIERNICZEGO PRZELEWU WIERZYTELNOŚCI</p>
...
<div class="pdf_strona">
    <?php } ?>
        ...
        <div class="pdf_szara_kratka pdf_kratka_duza">
            ...
            <div class="form-group col-md-12">
                <label class="pdf_duze_litery font_size_10">FIRMA PRZEDSIĘBIORCY</label>
                <div class="pdf_kratka"><?php echo (${'reprezentant_'.$i}['Nazwa'] == '') ? ${'reprezentant_'.$i}['Imie'].' '.${'reprezentant_'.$i}['Nazwisko'] : ${'reprezentant_'.$i}['Nazwa']; ?></div>
            </div>
            <div class="form-group col-md-4">
                <label class="pdf_duze_litery font_size_10">ULICA</label>
    ...
    <p class="margin_b_0">Cedent zbywa odpłatnie na rzecz Cesjonariusza wszelkie wierzytelności z tytułu <span class="font_w_700"><?php echo $umowa_dane['TytulOgraniczeniaWartosc']; ?></span>, przysługujące Cedentowi od Ubezpieczyciela: <span class="font_w_700"><?php echo $umowa_dane['UbezpieczycielNazwa']; ?></span>
        ...
        w wyniku której uszkodzeniu uległ pojazd marki <span class="font_w_700"><?php echo $umowa_dane['Marka'].' '.$umowa_dane['Model']; ?></span>, o nr rej. <span class="font_w_700"><?php echo $umowa_dane['NrRejestracyjny']; ?></span>, a Cesjonariusz przyjmuje
        wierzytelności i zobowiązuje się do podjęcia czynności zmierzających do uzyskania od dłużnika świadczeń z tytułu wierzytelności w imieniu własnym, lecz na rachunek Cedenta.</p>
    ...
    <!--        <p class="margin_b_0">Cedent zbywa odpłatnie na rzecz Cesjonariusza wszelkie wierzytelności jakie przysługują Cedentowi z tytułu odszkodowania za szkodę w pojeździe marki
            ...
            należnego w związku ze zdarzeniem z dnia <span class="font_w_700"><?php /*echo $umowa_dane['DataSzkody']; */?></span> (nr akt szkodowych <span class="font_w_700"><?php /*echo $umowa_dane['NumerAkt']; */?></span>), a Cesjonariusz przyjmuje
            wierzytelności i zobowiązuje się do powzięcia czynności zmierzających do uzyskania od Dłużnika świadczeń z tytułu wierzytelności w imieniu własnym, lecz na rachunek Cedenta.</p>
    -->
    ...
    <p class="margin_b_0">1. Za wykonanie czynności powierniczych Cesjonariusz otrzyma wynagrodzenie w postaci prowizji od uzyskanych na rachunek Cedenta
        świadczeń, której wysokość i sposób zapłaty określa załącznik nr 1 do niniejszej umowy.</p>
    <p class="margin_b_0">2. Niezależnie od ust. 1, wynagrodzenie Cesjonariusza stanowią także odsetki ustawowe za opóźnienie, naliczone na rzecz Cedenta,
        w wysokości określonej decyzją zakładu ubezpieczeń bądź prawomocnym orzeczeniem sądu.</p>
    <p class="margin_b_0">3. Informacja, o której mowa w ust. 1 stanowi tajemnicę handlową i nie podlega ujawnieniu bez zgody Cesjonariusza.</p>
    ...
    <?php } ?>
        ...
        <p class="margin_b_0">4. Cesjonariusz zobowiązuje się do przekazania na rzecz Cedenta kwoty uzyskanej od Dłużnika po potrąceniu prowizji, o której mowa
            w ust. 1 , w terminie 7 dni roboczych od daty uzyskania należności od dłużnika, na rachunek bankowy wskazany przez Cedenta lub
            w inny sposób określony w załączniku nr 1 do niniejszej umowy.</p>
    ...
    <p class="margin_b_0">1. Cedent oświadcza, że łączna suma odszkodowania, wypłacona przez Dłużnika osobie uprawnionej do uzyskania odszkodowania
        w związku ze szkodą, o której mowa w § 1 wyniosła do dnia zawarcia umowy <span class="font_w_700"><?php echo $umowa_dane['OtrzymanaKwotaWierzytelności']; ?></span> zł brutto
        (słownie: <span class="font_w_700"><?php echo $umowa_dane['OtrzymanaKwotaWierzytelnościSlownie']; ?></span> złotych).</p>
    <p class="margin_b_0">2. Cedent oświadcza, że szkoda, o której mowa w § 1 miała miejsce w okolicznościach, o których zawiadomił ubezpieczyciela i nie
    ...
    <p class="margin_b_0">3. Cedent oświadcza, że koszty naprawy pojazdu wynikające ze szkody, o której mowa w § 1 nie zostały pokryte na podstawie przedstawienia
        Dłużnikowi rachunku, faktury lub innego dokumentu potwierdzającego wysokość kosztów naprawy. Koszty, o których mowa
        w zdaniu pierwszym nie dotyczą holowania, najmu pojazdu zastępczego, parkowania, ani przedmiotów przewożonych w pojeździe,
        o którym mowa w § 1.</p>
        <p class="margin_b_0">4. Cedent oświadcza, że nie zrzekł się roszczeń przysługujących mu względem Dłużnika w związku ze szkodą, o której mowa w § 1,
            w tym w szczególności na podstawie ugody lub innego porozumienia.</p>
        <p class="margin_b_0">5. Cedent oświadcza, że wierzytelność, o której mowa w mowa w § 1 została zaspokojona wyłącznie w kwocie określonej w kosztorysie
            lub oświadczeniu ubezpieczyciela, które przekazał Cesjonariuszowi.</p>
        <p class="margin_b_0">6. Cedent oświadcza, że wszelkie wierzytelności określone w § 1, są wolne od obciążeń, oraz że uprawnienie do ich zbycia na rzecz
            osób trzecich nie zostało wyłączone.</p>
        ...
        <p class="margin_b_0">8. Cedent oświadcza, że ujawnił Cesjonariuszowi wszelkie nienaprawione uszkodzenia, jakie pojazd posiadał przed powstaniem szkody
            komunikacyjnej, o której mowa § 1. Ponadto Cedent oświadcza, że pojazd ten bezpośrednio przed powstaniem szkody był dopuszczony
            do ruchu po drogach publicznych, o ile nie złożył Cesjonariuszowi przed zawarciem umowy odmiennego oświadczenia w formie pisemnej
            albo nie przedłożył dokumentów stwierdzających okoliczności przeciwne.</p>
        <p class="margin_b_0">9. Cedent oświadcza, że Dłużnik, o którym mowa w § 1, w dacie zawarcia umowy nie ma względem niego żadnej wymagalnej Wierzytelności
            podlegającej potrąceniu z wierzytelnościami, o których mowa w § 1.</p>
        <p class="margin_b_0">10. Cedent zobowiązuje się do sporządzenia pisemnego zawiadomienia dłużnika o przeniesieniu wierzytelności i złożenia go na ręce
            Cesjonariusza w celu przedłożenia Dłużnikowi. Cedent zobowiązuje się powstrzymać od cofnięcia złożonego oświadczenia.</p>
        <p class="margin_b_0">11. W przypadku uzyskania przez Cedenta od Dłużnika świadczenia tytułem spłaty wierzytelności, określonej w § 1, po podpisaniu przez
            Cedenta niniejszej umowy, Cedent zobowiązuje się do przekazania całości świadczenia na rachunek bankowy Cesjonariusza ING Bank
            Śląski S.A. 19 1050 1575 1000 0023 1250 6369 oraz powiadomić o tym Cesjonariusza na piśmie w terminie 7 dni roboczych od daty
            uzyskania świadczenia.</p>
    ...
    <label class="pdf_duze_litery text_a_center col-md-12 margin_b_0 font_w_700 margin_t_20">PRAWA I OBOWIĄZKI STRON</label>
        ...
        <p class="margin_b_0">d) kosztorys wyceny szkody zawierający szczegółowy wykaz części podlegających wymianie lub naprawie, procedury naprawcze
            i czas ich wykonania oraz przyjęte ceny wyżej wymienionych, dla szkody, o której mowa w § 1 niniejszej umowy wykonany na zlecenie
            lub przez Dłużnika o którym mowa w § 1;</p>
        <p class="margin_b_0">e) wszelkie oświadczenia Dłużnika o przyznaniu odszkodowania z tytułu szkody o której mowa w § 1 lub potwierdzenie wpływu
            odszkodowania na rachunek bankowy Cedenta bądź pokwitowanie jego odbioru;</p>
        <p class="margin_b_0">f) w przypadku gdy wierzytelności przysługują Cedentowi z tytułu ubezpieczenia AUTO-CASCO – kopię polisy oraz ogólnych
            warunków umowy ubezpieczenia aktualnych na dzień zawarcia umowy.</p>
        <p class="margin_b_0">2. Cesjonariusz zobowiązuje się do wykorzystania przekazanych dokumentów wyłącznie w celu realizacji umowy.</p>
        <p class="margin_b_0">3. Cesjonariusz może wykonywać czynności wchodzące w zakres umowy za pomocą podmiotów współpracujących, w szczególności
            adwokatów lub radców prawnych, przy czym za działanie tych osób Cesjonariusz odpowiada wobec Cedenta, jak za działania własne.</p>
        <p class="margin_b_0">4. Koszty uzyskania świadczeń na rachunek Cedenta ponosi Cesjonariusz.</p>
        ...
        <p class="margin_b_0">1. Umowa zostaje zawarta na czas do całkowitego wyegzekwowania przez Cesjonariusza roszczeń dochodzonych na rachunek Cedenta
            w postępowaniu przedsądowym, sądowym i egzekucyjnym.</p>
        <p class="margin_b_0">2. Cesjonariusz zastrzega, że decyzję o dochodzeniu roszczeń na drodze sądowej podejmie w oparciu o przeprowadzoną ocenę
            ryzyka procesowego.</p>
        <p class="margin_b_0">3. W przypadku rozwiązania bądź wypowiedzenia umowy przed całkowitym wyegzekwowaniem roszczeń Cesjonariusz dokona na rzecz
            Cedenta w terminie 7 dni cesji zwrotnej wierzytelności w części, w której nie została zaspokojona w wykonaniu umowy.</p>
        ...
        <p class="margin_b_0">1. Wszelkie zmiany niniejszej umowy wymagają formy pisemnej pod rygorem nieważności.</p>
        <p class="margin_b_0">2. W sprawach nieuregulowanych stosuje się odpowiednio przepisy Kodeksu Cywilnego.</p>
    ...
    <?php } ?>
        ...
        <p class="margin_b_0">4. Umowa wraz z załącznikiem nr 1 została sporządzona w dwóch jednobrzmiących egzemplarzach, po jednym dla każdej ze Stron.</p>
        ...
        <div class="pdf_podpisy col-md-12 paddding_l_0 paddding_r_0 margin_t_60 margin_b_20">
            ...
            <div class="pdf_podpisy_p float_r col-md-6 paddding_l_0 paddding_r_0">
                <div class="pdf_kreska col-md-12 paddding_l_0 paddding_r_0 margin_b_0"></div>
                <p class="margin_b_0 font_size_10 pdf_duze_litery text_a_right">CEDENT <span class="font_size_8">(OSOBY DOKONUJĄCE CESJI POWIERNICZEJ)</span></p>
            </div>
            <div class="clear_b"></div>
        </div>


<!--        <div class="pdf_kreska col-md-12 paddding_l_0 paddding_r_0 margin_t_0 margin_b_10"></div>-->
<!--        <p class="margin_b_0 font_size_10">Zgodnie z art. 24 ust. 1 ustawy z dnia 29 sierpnia 1997 r. o ochronie danych osobowych (tekst jednolity: Dz.U. z 2016 r., poz. 922 ze zm.) VOTUM S.A. informuje, że:</p>-->
...
<!--        <p class="margin_b_0 font_size_10">2. dane osobowe będą przetwarzane w celu wykonania umowy, a za zgodą osoby której dane dotyczą, także w celach marketingowych.</p>-->
<!--        <p class="margin_b_0 font_size_10">3. dane osobowe mogą być przekazywane podmiotom współpracującym przy jej wykonaniu, jak również podmiotom, od których będą uzyskiwane informacje niezbędne do-->
<!--            wykonania umowy i podmiotom, od których będą dochodzone roszczenia,</p>-->
<!--        <p class="margin_b_0 font_size_10">4. posiada Pani/Pan prawo dostępu do treści danych oraz ich poprawiania, a także prawo wniesienia sprzeciwu wobec przetwarzania Pani/Pana danych w celach marketingowych,</p>-->
<!--        <p class="margin_b_0 font_size_10">5. podanie VOTUM danych osobowych jest dobrowolne.</p></br>-->
<!---->
<!--        <p class="margin_b_0 font_size_10">Cedent żąda rozpoczęcia czynności dochodzenia wierzytelności przez VOTUM przed upływem terminu do odstąpienia od umowy.</p>-->
