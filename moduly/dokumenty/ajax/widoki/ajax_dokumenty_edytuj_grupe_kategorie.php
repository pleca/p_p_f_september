...
$element = dokumenty_grupa_kategoria_pobierz_dane($element_id, $tabelka);

?>

<div class="szczegoly_dokumentu_tlo"></div>
<div class="szczegoly_dokumentu_przod edytuj_element">
	<div class="szczegoly_dokumentu_przod_belka_top">
		<p class="szczegoly_dokumentu_przod_belka_top_tytul" data-tabelka="<?php echo $tabelka; ?>" data-element_id="<?php echo $element_id; ?>">
			<?php echo $element['nazwa']; ?> (<?php echo $element['id']; ?>)
			
		</p>
		<span class="szczegoly_dokumentu_zamknij">X</span>	
		<?php 
		if($tabelka == 'dokumenty_strona'){
			if(in_array('21', $luzu)){ ?>
				<span style="display:block; float:right; " class="dokumenty_strona_edytuj_usun" ></span>
		<?php 
			}
		} ?>
		
		<?php 
		if($tabelka == 'dokumenty_kategoria'){
			if(in_array('24', $luzu)){ ?>
				<span style="display:block; float:right; " class="dokumenty_kategoria_usun" ></span>
		<?php 
			}
		} ?>	
		<div class="czy_jestes_pewnien">
			...
			<?php 
				if($tabelka == 'dokumenty_strona'){ echo ' dokumenty_strona_edytuj_usun_tak '; }
				if($tabelka == 'dokumenty_kategoria'){ echo ' dokumenty_kategoria_usun_tak '; }
			?>
	...
	if(in_array('20', $luzu)){
			...
			echo '<div class="dokumenty_strona_edytuj" data-strona_id="'.$element['id'].'">';
			echo '<input class="dokumenty_strona_edytuj_nr_kolejnosci " type="text" value="'.$element['nr_kolejnosci'].'" placeholder="Nr" />';
			echo '<input class="dokumenty_strona_edytuj_nazwa " type="text" value="'.$element['nazwa'].'"  placeholder="Nazwa" />';
			
			echo '<span style="display:block; float:left; " class="dokumenty_strona_edytuj_zapisz" >ZAPISZ</span>';
	...
	<?php 
				...
				$uprawnienie_element = dokumenty_grupa_kategoria_pobierz_grupy_po_id($element_id, $tabelka);
	...
	?>
				<div class="dokumenty_grupy_lista">
					...
					<div class="dokumenty_uzytkownicy_lista dokumenty_uzytkownicy_lista_edytuj_kategorie">
					...
					<div class="dokumenty_uzytkownicy_dodaj_nowy">DODAJ</div>
						<div id="dokumenty_uzytkownicy_lista_dodanych" class="dokumenty_uzytkownicy_lista_dodanych">
							<?php 
								...
								//echo $tabelka;
								if($tabelka == 'dokumenty_strona'){
									$lista_uzytkownikow = dokumenty_grupa_uzytkownicy_pobierz($element_id);
								}else{
									$lista_uzytkownikow = dokumenty_kategoria_uzytkownicy_pobierz($element_id);
								}
