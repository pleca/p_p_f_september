(function(factory) {
                                  ...
                                  on_keydown, on_keypress, on_keyup, on_autocomplete )
    {
        ...
        var content_insertimage = function(wysiwygeditor)
        {
            ...
            {
                var html = '<img id="wysiwyg-insert-image" style="margin-top: 10px; margin-bottom: 10px;" src="" alt=""' + (filename ? ' title="'+html_encode(filename)+'"' : '') + '>';
            ...
            // Create popup
            var $content = $('<div/>').addClass('wysiwyg-toolbar-form')
            							.addClass('dodaj_obraz_dad_kontener')
                                      .prop('unselectable','on');
            // Add image via 'Browse...'
            
            var $fileuploader = null,
                $fileuploader_input = $('<input class="dodaj_obraz_pole" type="file" accept="image/*">');
            ...
            $content.append( $('<div class="wstaw_z_url"/>').append($inputurl).append($okaybutton) );
            $content.append('<div class="wyswig-browse-tyl"></div>');
        ...
        var popup_position = function( $popup, $container, left, top )  // left+top relative to $container
        {
            ...
            if($popup.children().hasClass('dodaj_obraz_dad_kontener')){
            	$popup.addClass('wysiwyg-popup_dodaj_obraz');
            }
        };
        ...
        else
        {
            if( ! $editor )
            	...
            	$(".wysiwyg-editor").addClass('nowy') ;
            	$(".wysiwyg-editor").attr('id','wysiwyg_edytor') ;
            	//$(".wysiwyg-editor").append('<p>Wprowadź tekst...</p>');
            	
            	/*$('.wysiwyg-editor').bind('dragover drop', function(event){
            	    event.preventDefault();
            	    return false;
            	});*/
            	
            	$('.nowy').keydown(function(e){
            		var val = $('.nowy').text();
                ...
                var create_toolbar = function()
                {
                    ...
                    $(window).scroll(function() {
                        if ($(this).scrollTop() > 460) { 
                        	$toolbar.addClass('plywajacy_pasek');
                        }
                    	if ($(this).scrollTop() < 460) { 
                    		$toolbar.removeClass('plywajacy_pasek');
                        }
                    });
